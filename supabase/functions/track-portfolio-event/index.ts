...
type AnalyticsEvent = { name: string; dedupeKey?: string };
...
Deno.serve(async (request) => {
  ...
  const hashSalt = await sha256Hex(`portfolio-analytics|${serviceRoleKey}`);
  ...
  for (const item of events) {
    ...
    if (item.dedupeKey !== undefined && (typeof item.dedupeKey !== 'string' || item.dedupeKey.length < 8 || item.dedupeKey.length > 160)) {
  ...
  let accepted = 0;
  for (const item of events) {
    const dedupeHash = item.dedupeKey ? await sha256Hex(`${hashSalt}|dedupe|${item.name}|${item.dedupeKey}`) : null;
